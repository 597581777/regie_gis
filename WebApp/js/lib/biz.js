...
biz.utils = {
    ...
    switchTheme:function(csstheme){
        ...
        $.post(window.location.href,{theme:csstheme},function(){
 	 ...
 	 * @param value 设置组件的值  如此参数为空则返回组件值
 	 ...
 	 val:function(id,value){
 		...
 		if(value != "undefined" && value != null){
 			!!uiObj ? uiObj.setValue(value) : $("#"+this.id(id)).val(value) ;
 		}else{
    ...
    var i,j,n,
        ui = function(opt, _isInherits){// 创建新类的真构造器函数
           ...
           // opt = $.extend(true,{},me.defaults,opt) ;
            ...
            //把组件对象存放在biz.uiObj里
            if(opt.id != "undefined" && me.uiType != "validate"){biz.uiObj[$(opt.id).attr("id")] = me} ;
        },
        C = function(){};
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						//comboboxlist、comboboxtree组件需要在elc统计节点生成span标签，需要在elc完成append操作再初始化
                        if(this.edittype == "comboboxlist"){
                            var setting = $.extend(opt,{selected:tmp});
                            $(elc).dropdownlist(setting);
...
$.extend($.jgrid,{
        ...
        createEl : function(eltype,options,vl,autowidth, ajaxso ,v2) {
            ...
            switch (eltype)
            {
                ...
                case "comboboxlist" :
                    ...
                    //$(elem).dropdownlist(setting); 修改到savecell处理
                    break;
                case "comboboxtree" :
                    elem = document.createElement("input");
                    elem.type = "text";
                    ...
                    //new biz.comboboxtree(setting);修改到savecell处理
                    break;
                case "datepicker" :
                    elem = document.createElement("input");
                    elem.type = "text";
 ...
 $.fmatter.getDicData = function (cm){
     var dicData ,temp ;
 ...
 $.fn.fmatter.showlink = function(cellval, opts, rowData) {
        ...
        if(op.linksOptions.length> 0){
            for(var i=0;i<op.linksOptions.length;i++)(function(k){
                ...
                $(elem).attr("href",opt.baseLinkUrl+ cm + '?'+ opt.idName+'='+opts.rowId+pm);
                //按钮id，保证其唯一性
                $(elem).attr("id",opts.gid+"_"+opts.colModel.name+"_"+opts.rowId+"_"+k);
                if(opt.opeName) {
            ...
            return linkString;
        	 ...
        	 var cm = $.fmatter.replacePara(op.showAction,rowData,opts),
        	 	 ...
        	 	 elem = document.createElement("a");
             $(elem).attr("class","showlink");
             $(elem).attr("href",op.baseLinkUrl+ cm + '?'+ op.idName+'='+opts.rowId+pm);
             //按钮id，保证其唯一性
             $(elem).attr("id",opts.gid+"_"+opts.colModel.name+"_"+opts.rowId);
             if(op.opeName) {
 ...
 $.fn.fmatter.dictionary = function(cellval,opts,rwdat,act) {
     var colmodel = opts.colModel, cellname = "" ,tempArray ,
         data = $.fmatter.getDicData(colmodel),eidttype = opts.colModel.edittype;
     //数据为空提示用户
 ...
 biz.comboboxtree = biz.createUI(function(options){
    ...
    init:function(opts){
        ...
        function _beforeClick(treeId, treeNode,clickFlag) {
            if(opts._beforeClick){opts._beforeClick.call(this,treeId, treeNode,clickFlag);}
        ...
        function _setval(values){
        	var name = "",idKeys = $.isArray(values) ? values : values.split(",");
 ...
 */
        ...
        showErrors:function(){
			...
			for ( var i = 0; this.errorList[i]; i++ ) {
				...
				if($(error.element).hasClass("csiui-file")){
					var fileerrdiv = $('<div name="_diverr" style="border:1px solid #FF0000;width:auto; float: left;"></div>');
				...
				if($(error.element)[0].type == "checkbox"||$(error.element)[0].type == "radio"){
				    ...
				    if($(error.element).parent()!=null&&$(error.element).parent().parent()!=null&&$(error.element).parent().parent().parent()!=null){
				    	  if($(error.element).parent().parent().parent().attr("name")!="_diverr"){
						       var checkboxerrdiv = $('<div name="_diverr" style="border:1px solid #FF0000;overflow:hidden;width:auto; float: left;"></div>');
				    ...
				    if($(error.element).parent().attr("name")!="_diverr"){
						var selecterrdiv = $('<div name="_diverr" style="border:1px solid #FF0000;overflow:hidden;width:auto; float: left;"></div>');
					...
					if(isCblistOrCbtree){
						...
						if(hasSelectedtext){		
		                	if(isnullselectedvalue){
		                		var spanE = $(error.element).next();
		                    	var comboboxllisterrdiv = $('<div name="_diverr" style="border:1px solid #FF0000;overflow:hidden;width:auto; float: left;"></div>');
 ...
 biz.ajaxsubmit = biz.createUI(function(options){
     if(typeof options == "object"){
         ...
         defaults:{
         	isPostEmpty:true,
        	error:function(xhr, error, thrown){biz.utils.loadError(xhr,error, thrown)}
         },
         methods:{}
  });
