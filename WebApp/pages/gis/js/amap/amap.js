...
!function(global) {
    if (!global.AMap) throw new Error("璇峰墠缃紩鍏ラ珮寰峰湴鍥綼pi锛屽弬瑙乭ttp://lbs.amap.com/api/javascript-api/gettingstarted/");
}(window);
...
AMapUI.define("lib/SphericalMercator", [], function() {
    ...
    function project(lnglat) {
        var lat = Math.max(Math.min(maxs, lnglat[1]), -maxs), x = lnglat[0] * d, y = lat * d;
        y = Math.log(Math.tan(qdPI + y / 2));
        return [ x, y ];
    }
    ...
    function unproject(point) {
        var lng = point[0] * rd, lat = (2 * Math.atan(Math.exp(point[1])) - Math.PI / 2) * rd;
    ...
    function lngLatToPoint(lnglat, level) {
        return transform(project(lnglat), getScale(level));
    }
    function lngLatToPointByScale(lnglat, scale) {
        return transform(project(lnglat), scale);
    }
    ...
    var scaleCache = {}, d = Math.PI / 180, rd = 180 / Math.PI, qdPI = Math.PI / 4, maxs = 85.0511287798, a = .5 / Math.PI;
    return {
        getScale: getScale,
...
!function(ns, window) {
    ...
    ns.checkDomLibrary = function($, opts) {
        ...
        return !(libInfo.isJQuery && ns.versionCompare(libInfo.version, opts.minJQueryVersion) < 0) || "jQuery褰撳墠鐗堟湰(" + libInfo.version + ")杩囦綆锛岃鏇存柊鍒� " + opts.minJQueryVersion + " 鎴栦互涓婄増鏈紒";
    };
